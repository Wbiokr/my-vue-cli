...
export function open(id, url, sid) {
    ...
    if (isiOS && !window.mars) {
        if ($url != '' || $sid != '') {
            ...
            location.href = 'ydlc://h5/open?action=' + id + '&url=' + $url + '&id=' + $sid;
        }else {
        ...
        let idNum = Number(id);
        let mathVersion = '';
            mathVersion=window.ydwVersion.replace(/[^0-9]/ig,'');
        ...
        if ($url != '' || $sid != '') {
            window.mars.toAction(idNum,$url, $sid);
        } else {
